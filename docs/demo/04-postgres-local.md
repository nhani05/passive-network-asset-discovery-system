...
* **Kỳ vọng:** Chương trình lập tức báo lỗi và dừng:
  ```text
  error: --db-url has been removed; configure PostgreSQL with .env, DATABASE_URL, PG*, or DB_* environment variables
  ```

---

## 6. Chạy Chế Độ Không Ghi Nhận PostgreSQL (Tắt DB Integration)

Khi cần chạy thử nhanh trên terminal mà không muốn ghi đè dữ liệu vào PostgreSQL hiện có, sử dụng một trong các cách sau:

* **Cách 1: Chạy từ thư mục khác không chứa `.env`**
  ```bash
  cd /tmp && ~/vdt/passive-network-asset-discovery-system/build/asset-discovery --pcap ~/vdt/passive-network-asset-discovery-system/samples/arp.pcap --output table
  ```
* **Cách 2: Ghi đè biến môi trường khi chạy lệnh (Khuyên dùng)**
  ```bash
  env -u PGHOST -u PGPORT -u PGDATABASE -u PGUSER -u PGPASSWORD ./build/asset-discovery --pcap samples/arp.pcap --output table
  ```
* **Cách 3: Comment các dòng cấu hình trong file `.env`**
