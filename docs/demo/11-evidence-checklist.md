...
| **12** | Chạy đọc PCAP trong Docker container | `docker run --rm --network asset-net -v "$PWD/samples:/samples:ro" -e DB_HOST=db -e DB_PORT=5432 -e DB_NAME=asset_discovery -e DB_USER=postgres -e DB_PASSWORD=123456 passive-asset-discovery --profile pcap --pcap /samples/arp.pcap --output table` |
...
| **15** | Live Capture hoạt động thực tế trên host (hoặc log từ chối cờ cũ) | `sudo ./build/asset-discovery --profile live --interface eth0` |
| **16** | Kết quả đếm số dòng trong DB khi chạy lại lần 2 (Upsert) | `psql "..." -c "select count(*) from assets;"` |
| **17** | CLI/config defaults được giải thích và đối chiếu | `./build/asset-discovery --help`, `./build/asset-discovery --version`, `docs/demo/13-cli-parameters.md` |
