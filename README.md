...
| `asset-capture` | Capture child/helper dùng bởi pipeline capture; hiện có backend status và protocol self-test. |
...
`asset-capture` là process phụ cho pipeline capture. Ở trạng thái hiện tại, binary này dùng để kiểm tra capture backend và tự kiểm tra protocol message.
