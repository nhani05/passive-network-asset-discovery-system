...
void decodesArpEthernetFrame()
{
    ...
    expect(result.frame->payload.size() == 6, "payload should start after Ethernet header");
    expect(result.frame->payload.front() == 0x00, "payload first byte should be preserved");
    expect(result.frame->payload.back() == 0x04, "payload last byte should be preserved");
}
...
void rejectsTruncatedFrame()
{
    ...
    expect(result.error == EthernetDecodeError::TruncatedFrame, "truncated frame should return controlled error");
}
...
void decodesUnsupportedEtherType()
{
    ...
    expect(result.ok(), "unsupported EtherType should still decode as Ethernet");
    expect(result.frame.has_value(), "unsupported EtherType should produce a frame");
    if (!result.frame.has_value()) {
        return;
    }

    expect(result.frame->sourceMac == "02:42:ac:11:00:02", "unsupported EtherType keeps source MAC");
    expect(result.frame->destinationMac == "ff:ff:ff:ff:ff:ff", "unsupported EtherType keeps destination MAC");
...
void decodesHeaderOnlyFrame()
{
    ...
    expect(result.frame->destinationMac == "01:02:03:04:05:06", "header-only destination MAC should decode");
    expect(result.frame->sourceMac == "0a:0b:0c:0d:0e:0f", "header-only source MAC should decode");
    expect(result.frame->etherType == 0x0800, "header-only EtherType should decode");
    expect(result.frame->payload.empty(), "header-only Ethernet frame should have empty payload");
}
